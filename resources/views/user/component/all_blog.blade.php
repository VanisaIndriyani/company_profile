@if (count($articles) != 0)
  @foreach ($articles as $article)
      <div class="section-header mt-3">
        <div class="mb-3">
          @if($article->slug)
            <a href="#" class="decoration-none btn-show-blog" data-url="{{ route('ajax.blog.show', $article->id) }}">
        ...
        @if($article->slug)
          <a href="#" class="ml-3 btn btn-outline-primary btn-sm btn-show-blog" data-url="{{ route('ajax.blog.show', $article->id) }}">
            <span class="text-primary">Read More <i class="fa fa-long-arrow-right"></i> </span>
          </a>
        @endif
        <hr class="mt-3">
      </div>
